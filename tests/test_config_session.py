...
def test_session_keeps_disk_untouched_until_commit(fake_config_file):
    session = config_session.ConfigSession(fake_config_file, json_file.load_json_data(fake_config_file))

    session.write("search.accept_threshold", 42)

    assert session.read("search.accept_threshold") == 42
    assert json_file.load_json_value(fake_config_file, "search.accept_threshold") == 90
...
def test_session_commit_persists_and_clears_backup(fake_config_file):
    ...
    assert json_file.load_json_value(fake_config_file, "search.accept_threshold") == 42
...
def test_session_backup_holds_last_known_good_before_first_write(fake_config_file):
    ...
    assert json_file.load_json_value(backup_file, "search.accept_threshold") == 90
...
def test_uncommitted_edit_does_not_survive_simulated_crash(fake_config_file):
    ...
    fresh_disk_value = json_file.load_json_value(fake_config_file, "search.accept_threshold")
