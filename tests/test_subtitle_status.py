...
def test_summary_numerator_does_not_double_count_auto_then_manual() -> None:
    auto = _make_subtitle(SubtitleStatus.AUTO_DOWNLOAD)
    ...
    auto.status = SubtitleStatus.MANUAL_DOWNLOAD
    assert _downloaded_count(evaluated) == 1


def test_summary_counts_distinct_downloads() -> None:
    evaluated = [
        _make_subtitle(SubtitleStatus.AUTO_DOWNLOAD),
        _make_subtitle(SubtitleStatus.MANUAL_DOWNLOAD),
