...
class SettingsFlow(Flow):
    def run(self) -> None:
        log.event(LogEvent.BANNER, title="Opening UI")
        from subsearch.ui.application import open_settings_window

        manual_accepted = open_settings_window(search_worker_factory=self._make_search_worker)
...
class PipelineSearchFlow(Flow):
    def run(self) -> None:
        ...
        if not self.bootstrap.all_providers_disabled():
            log.event(LogEvent.BANNER, title=f"Searching on {self._enabled_provider_names()}")
