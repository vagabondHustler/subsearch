...
# Mostly ConfigKey members, but the per-token tuning rows register composite leaf paths
# (search.token_weights.title) that have no dedicated enum member, so plain strings are allowed.
DefaultsMap = list[tuple[ConfigKey | str, object]]


def _resolve_default(config_key: ConfigKey) -> object:
...
class SwitchRow(SettingRow):
    ...
    def __init__(self, config_key: ConfigKey, store: SettingsStore, parent: QWidget | None = None) -> None:
